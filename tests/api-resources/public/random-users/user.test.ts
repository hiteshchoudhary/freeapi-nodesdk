...
describe('resource user', () => {
  test('random', async () => {
    const responsePromise = client.public.randomUsers.user.random();
  ...
  test('random: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.public.randomUsers.user.random({ path: '/_stainless_unknown_path' })).rejects.toThrow(
