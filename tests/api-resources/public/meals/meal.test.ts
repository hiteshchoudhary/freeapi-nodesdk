...
describe('resource meal', () => {
  test('random', async () => {
    const responsePromise = client.public.meals.meal.random();
  ...
  test('random: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.public.meals.meal.random({ path: '/_stainless_unknown_path' })).rejects.toThrow(
