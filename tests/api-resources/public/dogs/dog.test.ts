...
describe('resource dog', () => {
  test('random', async () => {
    const responsePromise = client.public.dogs.dog.random();
  ...
  test('random: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.public.dogs.dog.random({ path: '/_stainless_unknown_path' })).rejects.toThrow(
