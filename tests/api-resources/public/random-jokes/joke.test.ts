...
describe('resource joke', () => {
  test('random', async () => {
    const responsePromise = client.public.randomJokes.joke.random();
  ...
  test('random: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.public.randomJokes.joke.random({ path: '/_stainless_unknown_path' })).rejects.toThrow(
